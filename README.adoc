...
If you use Maven, visit the https://start.spring.io/#!type=maven-project&language=java&platformVersion=2.4.3.RELEASE&packaging=jar&jvmVersion=1.8&groupId=com.example&artifactId=producing-web-service&name=producing-web-service&description=Demo%20project%20for%20Spring%20Boot&packageName=com.example.producing-web-service&dependencies=web,web-services[Spring Initializr] to generate a new project with the required dependencies (Spring Web and Spring Web Services).
...
NOTE: Both the `pom.xml` and `build.gradle` files show additional build information, which
